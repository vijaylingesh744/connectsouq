...
function GICTCList({ listGICTC, handleEdit, handleDelete, paginationInfo, PreviousPage, NextPage, page, setPage }) {
      ...
      <table className="table">
        <thead>
          ...
          <th scope="col" style={{ background: "aliceblue", fontSize: "16px", borderRadius: "10px 0px 0px 15px" }}>S.No</th>
            <th scope="col" style={{ background: "aliceblue", fontSize: "16px" }}>Code</th>
            <th scope="col"style={{ background: "aliceblue", fontSize: "16px" }}>Actions</th>
          </tr>
        </thead>
        <tbody>
          {listGICTC.map((sub, index) => (
            <tr key={index}>
